...
subroutine cos_calc(gref, pftref, lonsib, latsib, &
    ...
    !...calculate COS
        ...
        ! depending on varcos_switch in namel_sibdrv
        ...
        if ((co2_assim > dzero) .and. (cost%cosm > dzero)) then
        ...
        enddo
        cost%cos_grnd_Berry = sum(cos_grnd_lay)
        ...
        ! COS soil uptake kuncat is temperature dependent following Ogee et al.,
        ! 2016, and fca is based on Meredith et al., 2018 ISME
       ...
       elseif (isagri) then
          ...
          kuncat = kuncat/5 !1.5d-5 !kuncat lower in agricultural soil (Kaisermann et al. 2018, SS)
          fca = 6500
       ...
       call cos_flux_solver_Ogee(&
           ...
           MAX(0.01, sscolt%vol_liq(1)+sscolt%vol_ice(1)), & ! soil moisture
       ...
       if (soilogee_switch) then ! if the Ogee switch in namel_sibdrv is turned on,
         ...
         cost%cos_soil = cost%cos_grnd_Berry
         !print*, 'Berry soil flux'
       endif

       !print*, '  '
       !print*, 'cosm          ', cosm*1d12
       !print*, 'cos_grnd_Berry      ', cost%cos_grnd_Berry*1d12
       ...
       ! flux units of cos_assim, cos_flux, cos_grnd_Berry, cos_grnd_Ogee:
       ...
       ! cos_grnd_Berry: uptake is positive
