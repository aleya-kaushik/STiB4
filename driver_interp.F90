...
real(r8) :: facswdwn   ! scaling factor based on cosz
    ...
    ! NOTE: should we add checks for NaNs (.ne. itself) for each driver variable just to be safe
    ! currently checkign only precip variables

    ! interpolate driver temperature
    gprogt%tm = facsibdrv*gprogt%tm1 + (1.-facsibdrv) * gprogt%tm2
    !if (gprogt%tm .ne. gprogt%tm) then
    !    gprogt%tm = rzero
    !endif    
