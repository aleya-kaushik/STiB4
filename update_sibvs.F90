...
!--------------------------------
! Prepare string for year
if (luc_vs_switch) then
    write(syear,1000) year
    1000 format (I4)
    close(1000)

    !Read in sibvs file
    inquire(file=trim(vs_file)//trim(syear)//'.nc',exist=exist)
    IF (.not. exist) THEN
    ...
    print*,'  ',trim(vs_file)//trim(syear)//'.nc'
    ENDIF

    !...Expecting text file for single point runs
    if (single_pt) then
        ! Open the file
        open(unit=32,file=trim(vs_file)//trim(syear)//'.nc',form='formatted')
        ...
        CHECK( nf90_open(trim(vs_file)//trim(syear)//'.nc',nf90_nowrite,ncid) )
