...
if (single_pt) then
    ...
    !!!!!!!!!!!!!!!!
    ! Linda's new code
    nrresidual = modulo(subgridcount, nchunks)

    if (rank .le. nrresidual) then
        olength = ceiling(subgridcount/dble(nchunks))
        start_count = (rank-1)*olength+1
        stop_count  = rank*olength
    else
        olength = floor(subgridcount/dble(nchunks))
        start_count = nrresidual*(olength+1)+1 + (rank-nrresidual-1)*olength
        stop_count  = nrresidual*(olength+1) + (rank-nrresidual)*olength
    endif
    ...
    ! !!!!!!!!!!!!!!!
    ! ! evs: split based on PFT
    !
    ! allocate(subgridpft(subgridcount))
    ! allocate(split_points(nchunks))
    !
    ! ! find number of pfts per subgridbox
    ! do i=1,subgridcount
    !     subgridpft(i) = count(sibvs(subgridref(i))%pftref(:)/=0)
    ! enddo
    !
    ! ! find split points
    ! subgridchunk = ceiling(sum(subgridpft) / dble(nchunks))
    ! split_index = 1
    ! pft_count = 0
    ! do i=1,subgridcount
    !     pft_count = pft_count + subgridpft(i)
    !     if (pft_count .ge. subgridchunk) then
    !         split_points(split_index) = i
    !         pft_count = 0
    !         split_index = split_index + 1
    !         if (modulo(i,2) .eq. 1) then
    !             pft_count = subgridpft(i)
    !         else
    !             pft_count = 0
    !         endif
    !     endif
    ! enddo
    ! split_points(nchunks) = subgridcount
    !
    ! ! select start and end point based on rank
    ! if (rank .eq.  1) then
    !     start_count_ES = 1
    !     stop_count_ES = split_points(1)
    ! else
    !     start_count_ES = split_points(rank-1) + 1
    !     stop_count_ES = split_points(rank)
    ! endif
    !
    ! print*,start_count,stop_count
    ! print*,start_count_LK,stop_count_LK
    ! print*,start_count_ES,stop_count_ES
    !print*,rank
    !print*,sum(subgridpft)
    !print*,split_points
